...
class Cell:
  ...
  def _getSlicesTraffic(self):
    st = {}
    for uid in self.attachedUEs:
      u = self.attachedUEs[uid]
      buf = u['ue'].getBuffer()
      rbs = Util.get_required_rbs(u['mcs'], buf, self.simulation.tti)
      s = u['ue'].slice.id
      if s not in st:
  ...
  def schedule(self):
    ...
    # First schedule RTX
    for uid in self.attachedUEs:
      haveRtx, hps = self.attachedUEs[uid]['ue'].checkRetransmissions()
      if haveRtx:
        for hp in hps:
          hp['uid'] = uid
   ...
   #       self.inter_scheduler.deduct_from_slice(usr['ue'].slice.id, amount)
    ...
    slice_share, rtx_scheduled = self.inter_scheduler.scheduleTTI(slice_traffic, available_rbs, self.buffered_rtxs)
    scheduled = {}
    user_scheduled = {}
    sent = {}
    data_sent = {}
